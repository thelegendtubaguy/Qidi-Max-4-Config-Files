...
- **Config-confirmed:** the inspected Max 4 SOC package `preinst` removes files under `/home/qidi/printer_data/config` unless their basenames are `timelapse.cfg`, `saved_variables.cfg`, `MCU_ID.cfg`, `fluidd.cfg`, `saved_variables.cfg.bak`, or `motion_accuracy_calibration.json`, then installs the packaged stock config tree. Optimized macro, installed-state, runtime-state, and auto-update checksum files are not retained by that package path.
- **Runtime-confirmed:** after installing TLTG `26.08.06.1` and then firmware `01.01.06.05`, an OrcaSlicer start using TLTG slicer G-code reached stock error cancellation with unhomed `Z5` and `Z200` moves; `~/tltg-optimized-macros/auto-update.sh --run` then reported `Auto-update check: initialized latest release state.`, establishing that its config-local checksum state was absent. Direct filesystem state and the exact firmware install path remain unresolved.
