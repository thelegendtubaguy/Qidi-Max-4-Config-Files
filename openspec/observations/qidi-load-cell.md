...
- **Repository-confirmed:** public calibration, developer direct capture, configuration diagnostics, and origin-cache capture remain hard-disabled in installed package `26.07.26.12`; staged preflight uses the GPIO-passive origin adapter and does not read configuration. Bounded origin capture records host/print-time call intervals, holds process-local ownership, validates homing state before and after capture, and forces shutdown on ownership or post-state uncertainty.
- **Unresolved:** origin-cache conversion age, deterministic placement inside each synchronous call interval, force-safe invalid-value classification, saturation behavior, and K-profile repeatability remain unproven.

Sanitized idle cadence evidence is stored in `openspec/changes/add-load-cell-pa-calibration/evidence/direct-read-cadence.json`; controlled under-force origin evidence is stored in `openspec/changes/add-load-cell-pa-calibration/evidence/origin-cache-under-force.json`. Host, MCU, and compiled-artifact hashes are recorded in `openspec/changes/add-load-cell-pa-calibration/reverse-engineering.md`.
