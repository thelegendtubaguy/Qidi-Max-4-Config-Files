...
Four initial controlled captures and three subsequent ten-pulse K `0.020` campaigns requested 40 Hz polling during stationary `0.4 mm` PLA extrusion at `215 °C`. Every successful pulse preserved XYZ and logical E, restored PA and smooth time, retained QIDI Box `slot4`, and was followed by successful stock `G28`; local call-start gaps reached `66.016 ms`. Thermal soaks of 180 and 300 seconds and extending the low-flow lead from `0.15` to `0.5` seconds did not produce repeatable low-to-high amplitudes or normalized transition metrics. One long-lead cycle inverted the analyzer polarity because its low-flow baseline had already approached the high-flow force level. Conversion age remains inseparable from physical pressure-response delay, so no deterministic conversion-time bound or PA sweep is accepted. `query_cs1237_begin(config=60)` is a stock reconfiguration command, not an accepted restoration path; its cache, freshness, zero-state, and post-restart behavior remain unvalidated.
