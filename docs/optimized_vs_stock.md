...
- Guarded installer patches set X/Y first-strike homing to `100 mm/s` while retaining the `55 mm/s` second strike, two-strike tolerance validation, retry behavior, `20 mm` retract, controller command ordering, and final macro backoff.
- A prior optimized `65 mm/s` X/Y speed migrates to `100 mm/s` only when the prior installed-state ledger proves installer ownership; an unrecorded or changed `65` remains operator-managed. The replacement ledger carries the original stock value so uninstall restores stock rather than `65`.
- `installer/klipper/qidi/homing.py` replaces QIDI `klippy/extras/homing.py` only when its firmware-scoped stock SHA-256 or a validated prior-managed payload permits it. Unknown vendor hotfixes and local source drift stop before backup or writes. X/Y controller-entry waits are `100 ms`, recovery waits are `50 ms`, and X/Y pre-home dwell is `250 ms`; Z retains the `1 s` dwell.
- The installer records the first accepted vendor `homing.py` bytes and mode, deploys the payload through same-directory atomic replacement, and verifies the installed hash. Reinstall and later payload upgrades retain that original preimage; uninstall restores it only while the live source is still managed and leaves unknown drift untouched.
- Installer backups and rollback journals include managed Python preimages alongside `config/`. Source-aware archives bind each allowlisted external file to its ID, destination, archive member, hash, and mode; restore and recovery-sentinel clearing verify both the config snapshot and declared external snapshots.
- Managed Python source changes create `printer_data/.tltg_optimized_klipper_restart_required` before writing. Interactive install, uninstall, and restore offer a Klipper service-process restart; declining leaves the verified files and marker pending. `--yes` and auto-update require the service restart after idle preflight. Moonraker `POST /machine/services/restart` must return Klipper to `ready` under a different positive process ID before the marker is cleared. A config-only change retains the existing configuration-restart path.
- Auto-update resolves a retained activation marker before it fetches or accepts a release checksum. A failed service activation leaves the marker and installed source in place and does not advance the update checksum; a later idle check retries activation.
